...
def get_user(user_id: str):
    ...
    user = database.user.find_one({'id': _id})
    return userEntity(user)


@users.post('/user/')
def create_user(user: UserIn):
    if database.user.find_one(dict(user)):
    ...
    database.user.insert_one(new_user, dict())
...
def update_user(user: UserIn):
    ...
    is_updated_document = database.user.find_one_and_update(_filter, update_values)
...
async def delete_user(token: dict = Body(..., example={'token': 'access token value'})):
    ...
    if database.user.find_one_and_delete({'_id': id}):
